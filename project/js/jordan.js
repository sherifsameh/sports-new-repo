...
class Jordan{
    async getJordans(){
        try {
            let result = await fetch("json/jordans.json"); 
            let data=await result.json();
            let jordans=data.items;
            jordans=jordans.map(function(jordan){
                const{price}=jordan.fields;
                const{id}=jordan.sys;
                const{image}=jordan.fields;
...
class UI{
    displayJordans(jordans){
        let result='';
        jordans.forEach(jordan => {
            result += `
                    <div class="card">
                        <img src=${jordan.image} alt="Denim Jeans" style="width:100%">
                        <p class="price">Price : $ ${jordan.price}</p>
                        <p><button class="cart-btn" data-id=${jordan.id}>Add to Cart</button></p>
                    </div>
            `
        })
        ...
        btns.forEach(btns=>{
            ...
            else
            {
                btns.addEventListener('click',(e)=>
                {
                  ...
                  //get bags from Bags
                  let caritem={...Storage.getjordans(id),amount: 1};
                  console.log(caritem);
...
class Storage
{
    static saveproduct(jordans){
        localStorage.setItem('jordans',JSON.stringify(jordans));

    }
    static getjordans(id)
    {
        let jordans=JSON.parse(localStorage.getItem ('jordans'));
...
document.addEventListener('DOMContentLoaded',function(){
    let jordans=new Jordan()
    let ui=new UI();
    //setup app
    ui.setupApp();
    jordans.getJordans().then(function(jordans){
        ui.displayJordans(jordans);
        Storage.saveproduct(jordans);
    }).then(()=>{
        ui.getBagButtons();
