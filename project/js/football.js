...
class Football{
    async getJordans(){
        try {
            let result = await fetch("json/footballs.json"); 
            let data=await result.json();
            let footballs=data.items;
            footballs=footballs.map(function(football){
                const{price}=football.fields;
                const{id}=football.sys;
                const{image}=football.fields;
...
class UI{
    displayJordans(footballs){
        let result='';
        footballs.forEach(football => {
            result += `
                    <div class="card">
                        <img src=${football.image} alt="Denim Jeans" style="width:100%">
                        <p class="price">Price : $ ${football.price}</p>
                        <p><button class="cart-btn" data-id=${football.id}>Add to Cart</button></p>
                    </div>
            `
        })
        ...
        btns.forEach(btns=>{
            ...
            else
            {
                btns.addEventListener('click',(e)=>
                {
                  ...
                  //get bags from Bags
                  let caritem={...Storage.getfootballs(id),amount: 1};
                  console.log(caritem);
...
class Storage
{
    static saveproduct(footballs){
        localStorage.setItem('footballs',JSON.stringify(footballs));

    }
    static getfootballs(id)
    {
        let footballs=JSON.parse(localStorage.getItem ('footballs'));
        return footballs.find(football =>football.id === id);
    }
...
document.addEventListener('DOMContentLoaded',function(){
    let footballs=new Football()
    let ui=new UI();
    footballs.getJordans().then(function(footballs){
        ui.displayJordans(footballs);
        Storage.saveproduct(footballs)
    }).then(()=>{
        ui.getBagButtons();
