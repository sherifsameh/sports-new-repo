...
class Perfium{
    async getJordans(){
        try {
            let result = await fetch("json/perfiums.json"); 
            let data=await result.json();
            let perfiums=data.items;
            perfiums=perfiums.map(function(perfium){
                const{price}=perfium.fields;
                const{id}=perfium.sys;
                const{image}=perfium.fields;
                return {price,id,image};
            });
            console.log(perfiums);
            return perfiums;
...
class UI{
    displayJordans(perfiums){
        let result='';
        perfiums.forEach(perfium => {
            result += `
                    <div class="card">
                        <img src=${perfium.image} alt="Denim Jeans" style="width:100%">
                        <p class="price">Price : $ ${perfium.price}</p>
                        <p><button class="cart-btn" data-id=${perfium.id}>Add to Cart</button></p>
                    </div>
            `
        })
        ...
        btns.forEach(btns=>{
            ...
            else
            {
                btns.addEventListener('click',(e)=>
                {
                  ...
                  //get bags from Bags
                  let caritem={...Storage.getperfim(id),amount: 1};
                  console.log(caritem);
...
class Storage
{
    static saveproduct(perfiums){
        localStorage.setItem('perfims',JSON.stringify(perfiums));

    }
    static getperfim(id)
    {
        let perfims=JSON.parse(localStorage.getItem ('perfims'));
        return perfims.find(perfim => perfim.id === id);
    }
...
document.addEventListener('DOMContentLoaded',function(){
    let perfiums=new Perfium()
    let ui=new UI();
    perfiums.getJordans().then(function(perfiums){
        ui.displayJordans(perfiums);
        Storage.saveproduct(perfiums);
